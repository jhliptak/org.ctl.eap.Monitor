...
public class eapMultiQuery
{
    ...
    public int getAppTransactions(eapClient client) throws Exception {
		...
		String trxObj = "-O "+obj;
		eapAttrList.addAll(ah.buildMonitor(trxObj+" -A number-of-aborted-transactions -U c -r trx.aborted"));
		eapAttrList.addAll(ah.buildMonitor(trxObj+" -A number-of-application-rollbacks -U c -r trx.rollbacks"));
		eapAttrList.addAll(ah.buildMonitor(trxObj+" -A number-of-committed-transactions -U c -r trx.committed"));
		...
		eapAttrList.addAll(ah.buildMonitor(trxObj+" -A number-of-resource-rollbacks -U c -r trx.resourceRollbacks"));
		eapAttrList.addAll(ah.buildMonitor(trxObj+" -A number-of-timed-out-transactions -U c -r trx.timedOut"));
		eapAttrList.addAll(ah.buildMonitor(trxObj+" -A number-of-number-of-transactions -U c -r trx.number"));
		return checkAttrList(eapAttrList);
    }
    ...
    public int getConnector(eapClient client, String protocol) throws Exception {
		...
		eapAttrList.addAll(ah.buildMonitor(obj+" -A bytesReceived -U c -r "+protocol+".bytesRec"));
		eapAttrList.addAll(ah.buildMonitor(obj+" -A bytesSent -U c -r "+protocol+".bytesSent"));
		eapAttrList.addAll(ah.buildMonitor(obj+" -A errorCount -U c -r "+protocol+".errors"));
		...
		eapAttrList.addAll(ah.buildMonitor(obj+" -A requestCount -U c -r "+protocol+".reqCount"));
		return checkAttrList(eapAttrList);
    }
    ...
    public int getDatabaseHealth(eapClient client) throws Exception {
		...
		for (String ds : datasources){
			...
			eapAttrList.addAll(ah.buildMonitor(dsObj+" -A TimedOut -w 10 -U c -r "+ds+".ds.timedOut"));
		...
		for (String ds : xadatasources){
			...
			eapAttrList.addAll(ah.buildMonitor(dsObj+" -A MaxWaitTime -w 10000 -r "+ds+".ds.maxWaitTime"));
			eapAttrList.addAll(ah.buildMonitor(eapAttrList,dsObj+" -A TimedOut -U c-w 10 -r "+ds+".ds.timedOut"));
    ...
    public int getJmsHealth(eapClient client) throws Exception{
		...
		for (String server : hqservers) {
			...
			for (String queue : queues){
				...
				eapAttrList.addAll(ah.buildMonitor(qObj+" -A message-count -U c -r "+queue+".jmsq.msgCount"));
				eapAttrList.addAll(ah.buildMonitor(qObj+" -A delivering-count -U c -r "+queue+".jmsq.deliveringCount"));
				eapAttrList.addAll(ah.buildMonitor(qObj+" -A scheduled-count -U c -r "+queue+".jmsq.scheduledCount"));
				eapAttrList.addAll(ah.buildMonitor(qObj+" -A messages-added -U c -r "+queue+".jmsq.msgAdded"));
			}
		}
		if(eapAttrList.isEmpty()){
			Map<String,String> eapAttr = new HashMap <String,String>();
		...
		for (String app : deployments){
			...
			for(String subdeployment : subdeployments) {
				...
				for (String sub: subSystems) {
					if (sub.matches("web")){
						...
						eapAttrList.addAll(ah.buildMonitor(webObj+" -A expired-sessions -U c -r "+subdeployment+".web.exp.sess"));
						eapAttrList.addAll(ah.buildMonitor(webObj+" -A rejected-sessions -U c -r "+subdeployment+".web.reject.sess"));
					...
					if (sub.matches("webservices")) {
						...
						for (String websvc: websvcs) {
							...
							eapAttrList.addAll(ah.buildMonitor(websvcObj+" -A fault-count -U c -r "+websvc+".ws.fault-count"));
							eapAttrList.addAll(ah.buildMonitor(websvcObj+" -A request-count -U c -r "+websvc+".ws.request-count"));
							eapAttrList.addAll(ah.buildMonitor(websvcObj+" -A response-count -U c -r "+websvc+".ws.response-count"));	
						}
						
					}
					if (sub.matches("jpa")){
						String jpaObj = "-O "+obj+"/subdeployment="+subdeployment+"/subsystem="+sub;
						eapAttrList.addAll(ah.buildMonitor(jpaObj+" -A completed-transaction-count -U c -r "+subdeployment+".jpa.trx.completed"));
						eapAttrList.addAll(ah.buildMonitor(jpaObj+" -A successful-transaction-count -U c -r "+subdeployment+".jpa.trx.success"));
					...
					if (sub.matches("ejb3")) {
						...
						for (String ejb: ejbs) {
							...
							eapAttrList.addAll(ah.buildMonitor(ejbObj+" -A invocations -U c -r "+ejb+".ejb.invocations"));
							...
							eapAttrList.addAll(ah.buildMonitor(ejbObj+" -A pool-create-count -U c -r "+ejb+".ejb.pool.create"));
			...
			for(String subsystem : subsystems) {
					if (subsystem.matches("web")){
						...
						eapAttrList.addAll(ah.buildMonitor(webObj+" -A expired-sessions -U c -r "+app+".web.exp.sess"));
						eapAttrList.addAll(ah.buildMonitor(webObj+" -A rejected-sessions -U c -r "+app+".web.reject.sess"));
    ...
    public int getJvmHealth(eapClient client) throws Exception {
    	...
    	eapAttrList.addAll(ah.buildMonitor(eapAttrList, memObj+" -A heap-memory-usage -U MB -C used -calc /TotalMemory -calc *100 -r UsedMemory% -w 95"));
    	eapAttrList.addAll(ah.buildMonitor(eapAttrList, memObj+" -A non-heap-memory-usage -U MB -C used -r NonHeap"));
